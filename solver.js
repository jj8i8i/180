onmessage = function(e) {
    try {
        ...
        console.error("Error in Solver Worker:", error);
...
class Solver {
    ...
    find(items, depth) {
        ...
        this.runUnaryOps(items, depth);
        if (this.level >= 3) this.runSigmaOps(items, depth);
        this.runBinaryOps(items, depth);
        
        this.memo.set(key, true);
    }
    
    runUnaryOps(items, depth) {
        // ... (full code in next block)
    }

    runBinaryOps(items, depth) {
        // ... (full code in next block)
    }
    
    runSigmaOps(items, depth) {
        // ... (full code in next block)
    }
    
    processSigma(startSet, endSet, remaining, depth) {
        // ... (full code in next block)
    }
...
class Solver {
    ...
    find(items, depth) {
        ...
        this.runUnaryOps(items, depth + 1);
        if (this.level >= 3) this.runSigmaOps(items, depth + 1);
        this.runBinaryOps(items, depth + 1);
        ...
        if (this.level >= 2) {
             for (let i = 0; i < items.length; i++) {
                ...
                if (item.value > 0) {
                    ...
                    this.find([...remaining, { value: sqrtVal, str: `\\sqrt{${item.str}}`, complexity: item.complexity + 5, derivation: { op: '√', inputs: [item], resultStr: `${this.formatNumber(sqrtVal)}` } }], depth);
                    if (sqrtVal > 0) {
                        const dblSqrtVal = Math.sqrt(sqrtVal);
                        this.find([...remaining, { value: dblSqrtVal, str: `\\sqrt{\\sqrt{${item.str}}}`, complexity: item.complexity + 6, derivation: { op: '√√', inputs: [item], resultStr: `${this.formatNumber(dblSqrtVal)}` } }], depth);
                    }
                }
            }
        }
        if (this.level >= 3) {
            for (let i = 0; i < items.length; i++) {
                ...
                if (item.value >= 0 && item.value <= 10 && item.value % 1 === 0) {
                     ...
                     this.find([...remaining, { value: factVal, str: `(${item.str})!`, complexity: item.complexity + 8, derivation: { op: '!', inputs: [item], resultStr: `${this.formatNumber(factVal)}` } }], depth);
                }
            }
        }
    }
    ...
    runBinaryOps(items, depth) {
        for (let i = 0; i < items.length; i++) {
            for (let j = i + 1; j < items.length; j++) {
                ...
                this.tryOp(a, b, '+', remaining, depth);
                this.tryOp(a, b, '-', remaining, depth); this.tryOp(b, a, '-', remaining, depth);
                this.tryOp(a, b, '*', remaining, depth);
                this.tryOp(a, b, '/', remaining, depth); this.tryOp(b, a, '/', remaining, depth);
                if (this.level >= 1) { this.tryOp(a, b, '^', remaining, depth); this.tryOp(b, a, '^', remaining, depth); }
                if (this.level >= 2) { this.tryOp(a, b, 'root', remaining, depth); this.tryOp(b, a, 'root', remaining, depth); }
            }
        }
    }
    ...
    runSigmaOps(items, depth) {
        ...
        for (let i = 0; i < (1 << n); i++) {
            ...
            for (let j = 0; j < (1 << remaining1.length); j++) {
                ...
                this.processSigma(startSet, endSet, remaining2, depth);
            }
        }
    }
    ...
    processSigma(startSet, endSet, remaining, depth) {
        ...
        for (const sBound of startBounds) {
            for (const eBound of endBounds) {
                ...
                if (start <= 0 || end > 20 || end-start > 15) continue;
                ...
                for (const pat of simplePatterns) {
                    ...
                    this.find([...remaining, newItem], depth);
                }
                ...
                if (remaining.length > 0) {
                    for (let k_idx = 0; k_idx < remaining.length; k_idx++) {
                        ...
                        for (const pat of complexPatterns) {
                             ...
                             if (sigmaResult === null || !isFinite(sigmaResult)) continue;
                             const newItem = { value: sigmaResult, str: `\\sum_{i=${strI}}^{${strEnd}} (${pat.s})`, complexity: sBound.complexity + eBound.complexity + k.complexity + pat.c, derivation: { op: 'Σ', inputs: [sBound, eBound, k], resultStr: `${this.formatNumber(sigmaResult)}` } };
                             this.find([...finalRemaining, newItem], depth);
                        }
                    }
                }
            }
    ...
    getBoundaryValues(items) {
        ...
        if (items.length === 2) {
            ...
            val = a.value - b.value; if (val > 0) results.push({ value: val, str: `(${a.str}-${b.str})`, complexity: a.complexity + b.complexity + 1, derivation: { op: '-', inputs: [a,b], resultStr: this.formatNumber(val) } });
            val = b.value - a.value; if (val > 0) results.push({ value: val, str: `(${b.str}-${a.str})`, complexity: a.complexity + b.complexity + 1, derivation: { op: '-', inputs: [b,a], resultStr: this.formatNumber(val) } });
    ...
    tryOp(a, b, op, remaining, depth) {
        let value, str, complexity, resultStr, inputs = [a,b];
        if ((op === '*' || op === '/') && b.value === 1) return;
        if (op === '^' && b.value === 1) return;
        if (op === '*' && a.value === 1) { inputs = [b,a]; a = inputs[0]; b = inputs[1]; }
        ...
        this.find([...remaining, { value, str, complexity, derivation }], depth);
    }
    
    factorial = (n) => (n <= 1 ? 1 : n * this.factorial(n - 1));
    formatNumber = (n) => parseFloat(n.toFixed(2));
    addParen = (item, op) => {
